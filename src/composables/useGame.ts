...
export const useGame = () => {
  ...
  const initializeGame = () => {
    ...
    const sequenceLength =
      GAME_CONFIG.DIFFICULTY_LEVELS[state.value.difficulty].sequenceLength;
    ...
    state.value.timerSecond =
      GAME_CONFIG.DIFFICULTY_LEVELS[state.value.difficulty].timerSeconds;
  ...
  const setDifficulty = (
    ...
    const wasTimerRunning =
      state.value.timerSecond > 0 && state.value.isPlaying;

    state.value.difficulty = difficulty;

    // Update timer according to new difficulty
    state.value.timerSecond =
      GAME_CONFIG.DIFFICULTY_LEVELS[difficulty].timerSeconds;
    ...
    const sequenceLength =
      GAME_CONFIG.DIFFICULTY_LEVELS[difficulty].sequenceLength;
    ...
    if (wasMemorizing) {
      ...
      state.value.gameResult = {
        result: state.value.gameResult.result,
        bool: true,
      };
    } else {
  ...
  const checkResult = () => {
    ...
    if (state.value.startTime) {
      state.value.totalSelectionTime = Math.round(
  ...
  const selectNumber = (numberRandom: IColorRandom) => {
    if (
      ...
      state.value.timerSecond === 0 &&
      !state.value.gameResult.bool
    ) {
      // Check if this item is already in the player's sequence
      const isAlreadySelected = state.value.playerSequence.some(
        (item) => item.name === numberRandom.name,
      );

      if (
        !isAlreadySelected &&
        state.value.playerSequence.length < state.value.originalSequence.length
      ) {
